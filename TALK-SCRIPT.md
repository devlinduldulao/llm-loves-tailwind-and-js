...
> The friction of CSS was never **aesthetic** — it was **locality**. Every reason to stop arguing against Tailwind is the same reason a language model generates it better: **co-location + constraints.** The machines already voted; here's why.
...
| Counterargs → adopt | 25–26 | 2:15 | 28:15 |
...
2. **Punchline** — "None — they change everything around it and hope it cascades." That joke is literally today's talk.
3. **Title** — The methodology wars are over, and the machines voted. Today: why that's architecture, not fashion.
4. **Bio** — 15 seconds. Who you are, why you care about this. → *swap in your real creds.*
5. **Methodology wars** — 15 years of OOCSS/BEM/CSS-in-JS all answered two questions: where do styles live, what do we name them.
6. **Machines voted** — v0, bolt, Lovable, Artifacts, shadcn all default to Tailwind. Not marketing — revealed preference.
7. **Reframe** — This was never about clean HTML. It's about how code is *understood* — by a teammate and by a next-token predictor. Same fix: locality.
...
11. **Co-location** — Tailwind puts it all in one place. No naming, no file-hopping, no cascade. The model sees the whole spec in one span.
...
15. **Reason 3 · Design tokens** — It picks `p-4` from a menu instead of hallucinating `padding: 13px`. Constraints guide, not limit.
...
19. **JS frameworks** — Components already co-locate template + logic. Tailwind adds the 3rd pillar. Encapsulation and utility-first are the same principle: locality.
...
24. **Proof · shadcn** — Copy-paste components in Tailwind + TS + cva. It exists *because* devs and LLMs both read/modify it trivially. QED.
25. **Counterarguments** — "Class soup" → extract components. "Learning curve" → closed set + IntelliSense. "Just inline styles" → no: tokens + variants + purge.
...
27. **Takeaways** — Friction = locality. LLMs love TW because it's co-located, closed-vocab, deterministic, everywhere. Components+utilities+types = one idea, three layers.
...
3. **SAY:** "A wrong variant is a red squiggle, not a 2 a.m. incident. TypeScript and Tailwind are the same constraint at two layers."
...
3. **SAY (land it):** "Three prompts, five lines, **zero other files touched.** Because the component is co-located, the model's edit is *local* — and a local edit is a safe edit. That's the whole talk."
...
Bio creds (slide 4) · event + date (slides 3/29) · `<your-handle>` socials + a real QR (slide 29) · confirm `npm run dev` works on the venue machine/network.
